...
async def inline(_, query: InlineQuery):
        ...
        try:
        	...
        	METHOD=  [ InlineQueryResultArticle( title="How to use ❓", input_message_content=InputTextMessageContent("Ex ; ```@TranslateWoBot how to use # hi```"),description="{Text}#{language code} ",thumb_url="https://tg-link.herokuapp.com/dl/0/AgADh60xG50-wFc.jpg")]
        	await query.answer( results=METHOD,  cache_time=2, switch_pm_text="Using method",switch_pm_parameter="start" )
        	return
        try:
